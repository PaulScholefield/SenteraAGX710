...
% Improved MATLAB Code with Error Handling
clear; close all; clc;
...
for fol = 1:numel(folders)
    ...
    if ~exist(ndviFolder, 'dir') || ~exist(rgbFolder, 'dir') || ~exist(stackedFolder, 'dir')
        warning('One or more directories do not exist for folder: %s', folders{fol});
        continue;
    end
    ...
    for num = 1:numFiles
        ...
        try
            ...
            EXIFinfo1 = imfinfo(fullfile(ndviFolder, filename));
            ...
            EXIFinfo2 = imfinfo(fullfile(rgbFolder, filename));
            ...
            warning('Error processing file: %s. Error message: %s', filename, ME.message);
            continue;
        end
    end
end
